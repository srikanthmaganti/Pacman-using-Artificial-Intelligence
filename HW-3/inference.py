...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for value in self.legalPositions:
                distance= util.manhattanDistance(value, pacmanPosition)
                if emissionModel[distance] > 0:
                    possible[value]=self.beliefs[value]*emissionModel[distance]
        possible.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        for particleposition in range(self.numParticles):
            self.particles.append(legal[particleposition % len(self.legalPositions)])
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            self.particles = []
            for eachparticle in range(self.numParticles):
                self.particles.append(self.getJailPosition())
        else:
            possibleweight = util.Counter()

            for pos in self.particles:
                dist = util.manhattanDistance(pacmanPosition, pos)
                if emissionModel[dist] > 0:
                    possibleweight[pos] +=  emissionModel[dist] #float(belief[pos]) * float(emissionModel[dist])

            if possibleweight.totalCount() != 0:
                possibleweight.normalize()
                self.particles = []
                for eachparticle in range(self.numParticles):
                    self.particles.append(util.sample(possibleweight))
            else:
                self.initializeUniformly(gameState)
        #util.raiseNotDefined()
    ...
    def elapseTime(self, gameState):
        ...
        for value_count in range(num_of_particles):
            position=self.particles[value_count]
            if not newPosDist[position]:
                newPosDist[position]=self.getPositionDistribution(self.setGhostPosition(gameState, position))
            self.particles[value_count]=util.sample(newPosDist[position])
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        total_particles=self.particles
        beliefdistribution = util.Counter()
        for eachparticle in total_particles:
            beliefdistribution[eachparticle] += 1
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        num_of_particles = 0
        self.particles = []
        while num_of_particles<self.numParticles:
            for lp in possible_positions:
                if num_of_particles < self.numParticles:
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] == None:
                    ...
                    k *= emissionModels[i][dist]
            allPossible[particle] += k
        if not any(allPossible.values()):
            self.initializeParticles()
        else:
            allPossible.normalize()
            particle = [util.sample(allPossible) for i in range(self.numParticles)]
            # for i in range(self.numParticles):
            #     particle.append(util.sample(allPossible))
...
def getPositionDistributionForGhost(gameState, ghostIndex, agent):
    ...
    newGhostIndex = ghostIndex+1
    ghostPosition = gameState.getGhostPosition(newGhostIndex)
