...
def enhancedFeatureExtractorDigit(datum):
    ...
    ## Here I am using the number of continous white regions as features
    ## oneRegion ---> There is one white Region                   (Possible Numbers 1,2,3,4,5,7)
    ## twoRegion ---> The digit image contains 2 white regions    (Possible Numbers 2,4,6,9)
    ## threeRegion ----> The digit image contains 3 white regions (Possible Number 8)
    ## here the number of white regions is an assumed feature it may vary according a persons handwriting
    """
    features =  basicFeatureExtractorDigit(datum)

    image = datum.getPixels()
    whiteRegions = 0
    visited = [[False for i in range(DIGIT_DATUM_WIDTH)] for j in range(DIGIT_DATUM_HEIGHT)]
    for i in range(DIGIT_DATUM_HEIGHT):
        for j in range(DIGIT_DATUM_WIDTH):
            if visited[i][j] == False and image[i][j] == 0:
                explore(image, visited, i, j)
                whiteRegions += 1
    if(whiteRegions == 1):
        features['threeRegion'] = 0
        features['twoRegion'] = 0
        features['oneRegion'] = 1
    if(whiteRegions == 2):
        features['threeRegion'] = 0
        features['twoRegion'] = 1
        features['oneRegion'] = 0
    if(whiteRegions == 3):
        features['threeRegion'] = 1
        features['twoRegion'] = 0
        features['oneRegion'] = 0
    return features

def explore(image, visited, i, j):
    visited[i][j] = True
    if(i-1>=0 and i-1 < (DIGIT_DATUM_HEIGHT)):
        if visited[i-1][j] == False and image[i-1][j] == 0:
            explore(image, visited, i-1, j)
    if(i+1>=0 and i+1 < (DIGIT_DATUM_HEIGHT)):
        if visited[i+1][j] == False and image[i+1][j] == 0:
            explore(image, visited, i+1, j)
    if(j-1>=0 and j-1 < (DIGIT_DATUM_WIDTH)):
        if visited[i][j-1] == False and image[i][j-1] == 0:
            explore(image, visited, i, j-1)
    if(j+1>=0 and j+1 < (DIGIT_DATUM_WIDTH)):
        if visited[i][j+1] == False and image[i][j+1] == 0:
            explore(image, visited, i, j+1)
...
def enhancedPacmanFeatures(state, action):
    ...
    """
    features = util.Counter()
    # counter with { <feature name> : <feature value>, ... }
    successor = state.generateSuccessor(0, action)
    ghostPositions = successor.getGhostPositions()
    pacmanPosition = successor.getPacmanPosition()
    remainingFood = successor.getFood()
    legalActions = successor.getLegalActions()
    capsules = successor.getCapsules()
    # Considering Manhattan Dist from each ghost as a feature
    for ghostNum, ghostPos in enumerate(ghostPositions):
        featureName = 'ghost' + str(ghostNum) + 'Distance'
        features[featureName] = util.manhattanDistance(pacmanPosition, ghostPos)

    # The ramaining food at each state is being considered as a feature
    features['remainingFood'] = remainingFood.count()
    xL = len(list(remainingFood))
    yL = [len(list(remainingFood)[i]) for i in range(xL)][0]
    if features['remainingFood']:
        foodDist = [util.manhattanDistance(pacmanPosition, (x, y)) for x in range(xL) for y in range(yL) if successor.hasFood(x, y)]
        features['closestFoodDist'] = min(foodDist)

    # Capsule features
    features['remainingCapsule'] = len(capsules)
    if features['remainingCapsule']:
        capsulesDist = [util.manhattanDistance(pacmanPosition, capsulePos) for capsulePos in capsules]
        features['closestCapsuleDist'] = min(capsulesDist) #closest capsule dist

    # Looking ahead with the Future action features
    totalFutureActions = len(legalActions)
    for futureAction in legalActions:
        futureSuccessor = successor.generateSuccessor(0, futureAction) # Generates a future successor
